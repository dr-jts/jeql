...
//	private boolean dataExtent = false;
	...
	/*
	public void setDataExtent(boolean calcExtent)
	{
		this.dataExtent = calcExtent;
	}
	*/
	...
	public void execute(Scope scope) throws Exception 
	{
		...
		for (int i = 0; i < num; i++) {
			...
			if (matcher != null 
					&& ! matcher.matchIgnoreCase(layerName))
				continue;
...
//			System.out.println(lyr.getName());
			...
			setValue(row, COL_COORDREF_INDEX, lyr.getCoordRef().getCoordSysDescription());
			...
			if (indexExtent) {
				...
				row.setValue(COL_INDEXEXTENT_INDEX, SdeUtil.toGeometry(lyr.calculateExtent(true, new SeSqlConstruct())));
			}
			int shapeTypeMask = lyr.getShapeTypes();
			setValue(row, COL_ALLOW_POINT_INDEX, new Boolean((shapeTypeMask & SeLayer.SE_POINT_TYPE_MASK) != 0));
			setValue(row, COL_ALLOW_LINE_INDEX, new Boolean((shapeTypeMask & SeLayer.SE_LINE_TYPE_MASK) != 0));
			setValue(row, COL_ALLOW_SIMPLE_LINE_INDEX, new Boolean((shapeTypeMask & SeLayer.SE_SIMPLE_LINE_TYPE_MASK) != 0));
			setValue(row, COL_ALLOW_AREA_INDEX, new Boolean((shapeTypeMask & SeLayer.SE_AREA_TYPE_MASK) != 0));
			setValue(row, COL_ALLOW_MULTIPART_INDEX, new Boolean((shapeTypeMask & SeLayer.SE_MULTIPART_TYPE_MASK) != 0));
			rl.add(row);
		}
		result = new Table(rl);
	}
