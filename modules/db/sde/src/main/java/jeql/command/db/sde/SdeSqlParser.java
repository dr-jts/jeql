...
//		System.out.println(condition);
	...
	private String[] parseColumns(String colList)
	{
		...
		for (int i = 0; i < cols.length; i++) {
			cols[i] = cols[i].trim();
			if (cols[i].length() <= 0) {
				throw new IllegalArgumentException("Blank column name found in SDE query");
			}
		}
		return cols;
	}
	...
	/**
	 * Gets the list of columns provided in the query string.
	 * If the query column list was "*", this is 
	 * returned as the single column name.
	 * 
	 ...
	 * The WHERE keyword is not returned.
	 * 
